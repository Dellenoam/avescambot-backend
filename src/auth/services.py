...
class AuthService:
    def __init__(
        self,
        unit_of_work: Type[AbstractUnitOfWork],
    ):
    ...
    async def create_new_user(self, user_to_create: UserCreate) -> UserAsResponse:
        ...
        async with self._unit_of_work as uow:
            if await uow.user.get_by(username=user_to_create.username):
                raise UserExistsError("User with this username already exists")
            if await uow.user.get_by(email=user_to_create.email):
            ...
            new_user = await uow.user.add(user_to_create_dict)
    ...
    async def validate_user(self, user: UserLogin) -> AuthUser:
        ...
        async with self._unit_of_work as uow:
            ...
            user_from_db = await uow.user.get_by(username=user.username)

            if not user_from_db:
                raise unauthenticated_exception

            if (
                validate_password_hash(user.password, user_from_db.hashed_password)
                is False
            ):
                raise unauthenticated_exception
...
class TokenService:
    def __init__(self, unit_of_work: Type[AbstractUnitOfWork]):
        self._unit_of_work = unit_of_work(async_session)
